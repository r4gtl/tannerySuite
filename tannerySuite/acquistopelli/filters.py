...
class LottoFilter(django_filters.FilterSet):
    ...
    identificativo = django_filters.CharFilter(
        field_name="identificativo",
    ...
    fk_tipoanimale = django_filters.ModelChoiceFilter(
        queryset=TipoAnimale.objects.all(),
        empty_label="Tutti i tipi animale",
        label="Tipo Animale",
        widget=forms.Select(attrs={"class": "form-control"}),
    )
    fk_tipogrezzo = django_filters.ModelChoiceFilter(
