...
def filtro_lotti_senza_totale(data_inizio, data_fine):
    lotti_filtrati = (
        Lotto.objects
        .filter(data_acquisto__range=[data_inizio, data_fine])
        .values('fk_fornitore__ragionesociale', 'data_acquisto', 'identificativo', 'fk_tipoanimale__descrizione', 'origine')
        .annotate(numero_lotti=Count('pk'))
        .order_by('fk_fornitore')
    )
...
def filtro_lotti(data_inizio, data_fine):
    lotti_filtrati = (
        Lotto.objects
        .filter(data_acquisto__range=[data_inizio, data_fine])
        .values('fk_fornitore__ragionesociale', 'fk_tipoanimale__descrizione', 'origine')
