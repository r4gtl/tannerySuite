...
def origine_old(request):
    ...
    queryset = (
        Lotto.objects.filter(data_acquisto__gte=twelve_months_ago)
        .values("origine")
        .annotate(lotti_count=Count("pk"))
    )
    result = {q["origine"]: q["lotti_count"] for q in queryset}
...
def origine(request):
    ...
    queryset = (
        ...
        .order_by(
            "-lotti_count"
        )  # Ordina per il numero di occorrenze in ordine decrescente
...
def origine_per_rpt_lwg(request):
    ...
    if request.method == "GET":
        ...
        queryset = (
            Lotto.objects.filter(data_acquisto__range=[from_date, to_date])
            .values("origine")
            .annotate(lotti_count=Count("pk"))
...
def tipoanimale_per_rpt_lwg(request):
    ...
    if request.method == "GET":
        ...
        result = {q["fk_tipoanimale__descrizione"]: q["numero_pezzi"] for q in queryset}
...
def tipogrezzo_per_rpt_lwg(request):
    ...
    if request.method == "GET":
        ...
        result = {q["fk_tipogrezzo__descrizione"]: q["numero_pezzi"] for q in queryset}
